...
namespace Games
{
	public class MenuCtlForPanel : MenuCtl
	{
		...
		private void OnLoadWindow(string filename, object obj)
		{
			if (obj == null)
			{
				Loger.LogErrorFormat("MenuItem OnLoadWindow obj=null, menuId={0}, menuName={1}, filename={2}", menuId, config.name, filename);
				return;
			}

			GameObject go = (GameObject) GameObject.Instantiate((GameObject)obj);
